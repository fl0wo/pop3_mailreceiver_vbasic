...
Public Function UUDecodeToFile(strUUCodeData As String, strFilePath As String)
    ...
    Dim vDataLine   As Variant      'Variabili per decodificare
    Dim vDataLines  As Variant
    Dim strDataLine As String
    Dim intSymbols  As Integer
    Dim intFile     As Integer
    Dim strTemp     As String
    If Left$(strUUCodeData, 6) = "inizio " Then  'Controllo se il file è "encoded"
        strUUCodeData = Mid$(strUUCodeData, InStr(1, strUUCodeData, vbLf) + 1)
    End If
    If Right$(strUUCodeData, 4) = "fine " + vbLf Then 'Controllo se la "fine" è disponibile
        strUUCodeData = Left$(strUUCodeData, Len(strUUCodeData) - 7)
    End If
    intFile = FreeFile
    Open strFilePath For Binary As intFile  'Apro il file output
        vDataLines = Split(strUUCodeData, vbLf)
        For Each vDataLine In vDataLines    'ogni riga
